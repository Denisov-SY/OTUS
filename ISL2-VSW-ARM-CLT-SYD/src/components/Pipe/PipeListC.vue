<template>
  <div class="row">
    {{`Здесь будет список труб. id штрипса: ${this.stripId}` }}
    <div class="col-12 py-1">
      <q-table flat bordered _title="Трубы" dense separator="cell" class="table-style1" :rows="Pipes" :columns="columns" :visible-columns="visibleColumns"
        row-key="PR_PIPE_ID" :filter="MyFilter" :filter-method="MyFilterMethod" @row-click="cellSelect" :rows-per-page-options="[5, 10, 15, 20, 25, 40, 50, 0]"
        
        v-model:pagination="MyPagination"
        selection="single" v-model:selected="pipeSelected"
        :selected-rows-label="(num) => 'Выбрана Труба: ' + pipeSelected[0].PIPE_NUMBER" ref="refPipeList">
...
export default {
  ...
  data() {
    return {
      ...
      visibleColumns: ["PIPE_NUMBER", "STATUS_NAME", "LAST_WC", "BATCH_NUMBER", "PROGRESSIVE", "JOB_CODE", "HEAT", "DIAM", "WALL_THICK", "PIPE_LENGTH", "STRENGTH_GROUP", "STANDARD", "SOURCE", "BIT_STATUS", "TIME_POSITION", "TIME_STATUS", "PRODUCTION_DATE",],
  ...
  methods: {
    ...
    // loadDataPipes: async () => {    // Загрузка даннных
    //   const PROC_NAME ="loadDataPipes>";
    //   // console.log("MyPagination.value= ", MyPagination.value);
    //   // if (!loading.value) s_loading(true, "Подождите, загружаются данные");  // 
  ...
  setup(props, context) {
    ...
    onMounted(async () => {
      const PROC_NAME = "PipeListC.onMounted> ";
